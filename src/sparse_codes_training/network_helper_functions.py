"""
This module is used for lower level functions on neural networks,
...
def find_divergences(base, rlhf, layer_name_stem: str, with_adapter=False):
    ...
    if not with_adapter:
        assert len(list(base.named_parameters())) == len(list(rlhf.named_parameters())), (
            'Base and rlhf should have same number of params!'
        )

    for (name_base, param_base), (_, param_rlhf) in zip(base.named_parameters(), rlhf.named_parameters()):
...
def find_layers(base, rlhf):
    ...
    if 'pythia' in model_name:
        ...
        raise ValueError(f'Finding divergence for {model_name} not supported.')
