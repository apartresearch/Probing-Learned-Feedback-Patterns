...
from sparse_codes_training.autoencoder_trainer_and_preparer import AutoencoderDataPreparerAndTrainer
from sparse_codes_training.metrics.mmcs import compare_autoencoders
from sparse_codes_training.network_helper_functions import find_layers
...
class ExperimentRunner:
    ...
    def initialize_models(
        ...
        if 'gpt-j' in policy_model_name:
            self.m_base = GPTJForCausalLM.from_pretrained(base_model_name, device_map="auto")
            self.m_rlhf = GPTJForCausalLM.from_pretrained(base_model_name, device_map="auto")
            self.m_rlhf.load_adapter(policy_model_name)

        else:
            self.m_base = AutoModel.from_pretrained(base_model_name).to(model_device)
            self.m_rlhf = AutoModel.from_pretrained(policy_model_name).to(model_device)
